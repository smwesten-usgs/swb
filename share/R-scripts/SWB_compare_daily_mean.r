...
pdf(file = "SWB_daily_mean_value_comparison_plot.pdf",width = 11, height = 8.5)
...
pdf(file = "SWB_daily_time_series_value_comparison_plot.pdf",width = 11, height = 8.5)
...
for (i in sort(unique(v_in$Year))) {
  ...
  for (j in 2:ncol(vb)) {
    ...
    if ( any(colnames(vbb)==colnames(vb)[j] ) ) {
      ...
      par(mar=c(3,6,3,3))
      plot(x=vb$Date, y=y, xlab="",ylab="", main=title_txt,col="red", type="p")
      lines(x=vbb$Date, y=x, col="blue")
      box(lwd=1.5)
