...
ylim.min <- min(mb_in$Mean.Min.Temp)
ylim.max <- max(mb_in$Mean.Max.Temp)
...
x<-as.vector(c(mb$Date,rev(mb$Date)))
ylim.min<-0.
ylim.max<-max(c(mb_in$Gross.Precip,
                mb_in$Net.Rainfall,
                mb_in$Snowmelt))
ylim.max<-ylim.max + 0.025 * (ylim.max - ylim.min)
ylim.min<-ylim.min - 0.025 * (ylim.max - ylim.min)
...
x<-as.vector(c(mb$Date,rev(mb$Date)))
ylim.min<-min(c(mb_in$TOTAL.Soil.Moisture.Storage,
                mb_in$TOTAL.Surface.Storage..snow.))
ylim.max<-max(c(mb_in$TOTAL.Soil.Moisture.Storage,
                mb_in$TOTAL.Surface.Storage..snow.))
ylim.max<-ylim.max + 0.025 * (ylim.max - ylim.min)
ylim.min<-ylim.min - 0.025 * (ylim.max - ylim.min)
...
x<-as.vector(c(mb$Date,rev(mb$Date)))
ylim.min<-min(mb_in$Actual.Evapotranspiration)
ylim.max<-max(mb_in$Actual.Evapotranspiration)
ylim.max<-ylim.max + 0.025 * (ylim.max - ylim.min)
ylim.min<-ylim.min - 0.025 * (ylim.max - ylim.min)
...
x<-as.vector(c(mb$Date,rev(mb$Date)))
ylim.min<-min(c(mb_in$Recharge,
                mb_in$Rejected.Recharge + mb_in$Recharge,
                mb_in$Surface.Flow.Out.of.Grid + mb_in$Rejected.Recharge 
                  + mb_in$Recharge))
ylim.max<-max(c(mb_in$Recharge,
                mb_in$Rejected.Recharge + mb_in$Recharge,
                mb_in$Surface.Flow.Out.of.Grid + mb_in$Rejected.Recharge 
                + mb_in$Recharge))
ylim.max<-ylim.max + 0.025 * (ylim.max - ylim.min)
