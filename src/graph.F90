     ...
     iY = INT(pGrd%iNY)
      ...
      select case(pGrd%iDataType)
        ...
        case(T_INT_GRID)
    	  ...
    	  do i=1,iX
	        do j=1,iY
		      ZMAT(i,j) = REAL(pGrd%iData((iY-j+1),i))
    		end do
    	  end do
        ...
        case(T_SGL_GRID)
    	  ...
    	  do i=1,iX
	        do j=1,iY
		      ZMAT(i,j) = REAL(pGrd%rData((iY-j+1),i))
    		end do
    	  end do
...
!       write(UNIT=LU_LOG,FMT=*) "pGraph%cCDEV: ",pGraph%cCDEV
!       write(UNIT=LU_LOG,FMT=*) "pGraph%cPAPER_SIZE: ", pGraph%cPAPER_SIZE
!       write(UNIT=LU_LOG,FMT=*) "pGraph%iWinSizeX: ",pGraph%iWinSizeX
!       write(UNIT=LU_LOG,FMT=*) "pGraph%iWinSizeY: ",pGraph%iWinSizeY
!       write(UNIT=LU_LOG,FMT=*) "pGraph%cSETFIL: ", pGraph%cSETFIL
!
!       write(UNIT=LU_LOG,FMT=*) "LEN pGraph%cSETFIL: ",LEN(pGraph%cSETFIL)
    ...
    ! ZOR, ZSTP 	:: first Z-axis label and the step between labels.
     ...
     select case(pGrd%iDataType)
       ...
       case(T_INT_GRID)
         ...
         do i=1,iX
	        do j=1,iY
		       ZMAT(i,j) = REAL(pGrd%iData((iY-j+1),i),kind=T_SGL_DISLIN)
           end do
         end do
