...
# Lysimeter data - two parts must be concatenated to form a complete dataset
...
```{r aggregate, message=FALSE, warning=FALSE, results='hide'}
dailyprcp <- summaryBy(DepthIncr ~ datePOSIX, data=prcp, FUN=c(sum))
dailyLysWt <- summaryBy(WEIGHT ~ datePOSIX, 
                        data=subset(lys,!is.na(datePOSIX)),
                        FUN=c(mean))
...
```{r writeGrids, message=FALSE, warning=FALSE, results='hide'}
if(writeDailyGrids) {
  ...
  for (i in seq(1956,2010)) {
    ...
    cat(file=filename, paste("Month","Day","Year",
                             "TAVG (F)",
                             "Precipitation (in)",
                             "Avg Rel Hum (%)",
                             "TMAX (F)",
    ...
    for (j in seq(1,length(Month)) ) {
      ...
      if( Year[j] == i ) {
        ...
        cat(file=filename,Month[j], Day[j], Year[j],
            TAVG[j], Precipitation[j], Avg_Rel_Hum[j],
            TMAX[j], TMIN[j], WINDVEL[j],
            Min_Rel_Hum[j], Pct_Sunshine[j],"\n", append=TRUE)      
      }
    }    
  }  
}
...
```{r plotRunoff, message=FALSE, warning=FALSE, results='hide'}
...
legend("topright", legend=c("Watershed 127","swb"),
       col=c("black","red"), pch=c(21,21), inset=c(0.02,0.02))
```
Now let's compare normalized soil moisture values recorded at the lysimeter with the values calculated by swb.

```{r plotSM, message=FALSE, warning=FALSE, results='hide'}
# normalize by assumuning that field capacity is approximated
# by the maximum value attained during the year
...
dailyLysWt$percentSat <- dailyLysWt$WEIGHT.mean / max(dailyLysWt$WEIGHT.mean,                                                     rm.na=TRUE)

plot(dailyLysWt$percentSat ~ dailyLysWt$datePOSIX,xlab="Date", 
     ylab="Fraction of Field Capacity", ylim=c(0,1),
     xlim=c(ymd("2001-01-01"),ymd("2001-12-31"))) 
points(ssf_sm$percentSat ~ ssf_sm$datePOSIX,col="red")
legend("bottomleft", legend=c("Lysimeter","swb"),
       col=c("black","red"), pch=c(21,21), inset=c(0.02,0.02))
```
The size of the root zone in the swb calculations appears to be too small; a larger total water capacity would attenuate the changes in soil moisture
